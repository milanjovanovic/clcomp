...
(defun operand-place-type (place)
  (let ((operand (operand-place-operand place)))
    (etypecase operand
      (keyword :register)
      (cons :stack))))
...
(defstruct ssa-block index order ir ir-last-cons ssa succ cond-jump uncond-jump
  ...
  exit cond-jump-moves uncond-jump-moves succ-moves spill-moves)
...
(defun instr-fun-call-p (instr)
  (typep instr 'ssa-fun-call))
...
(defparameter *debug-block-optimisation* nil)
(defmacro debug-blocks-print (&rest rest)
  (let ((ev (gensym)))
    `(let((,ev (list ,@(mapcan (lambda (x) (list `',x x)) rest))))
       (when *debug-block-optimisation*
	 (print ,ev))
       (car (last ,ev)))))
...
(defun lambda-ssa-add-block-after-block (lambda-ssa after-index block)
  (do ((tail (lambda-ssa-blocks lambda-ssa) (cdr tail)))
      ((null tail))
    (when (= (ssa-block-index (car tail)) after-index)
      (setf (cdr tail) (cons block (cdr tail)))
      (return))))

(defun lambda-ssa-clear-data-moves (lambda-ssa)
  (dolist (b (lambda-ssa-blocks lambda-ssa))
    (setf (ssa-block-cond-jump-moves b) nil)
    (setf (ssa-block-uncond-jump-moves b) nil)
    (setf (ssa-block-succ-moves b) nil)
...
(defun simple-insert-block-unconditional-jump (block to-block)
...
(defun ssa-block-add-instruction-to-start (ssa-block instructions)
  (let ((first-instr (ssa-block-first-instruction ssa-block)))
...
(defun ssa-block-add-instruction-to-end (ssa-block instructions)
  (let ((last-instr (ssa-block-last-instruction ssa-block)))
    ;; if last instruction is IF we don't know what to do
    (assert (not (typep last-instr 'ssa-if)))
    (if (or (typep last-instr 'ssa-go))
	(setf (ssa-block-ssa ssa-block)
	      (append (butlast (ssa-block-ssa ssa-block))
		      instructions
		      (list last-instr)))
	(setf (ssa-block-ssa ssa-block)
	      (append (ssa-block-ssa ssa-block)
		      instructions )))))
  ...
  (let* ((test-node (clcomp::if-node-test-form if-node))
	...
	(label-ssa-block false-block false-block-label))
      (emit-ir (make-ssa-if
		...
		:false-block-label (ssa-block-label false-block))
	       block)
      (simple-insert-block-unconditional-jump block false-block)
...
(defun emit-tagbody-node-ssa (node lambda-ssa leaf place block)
  ...
  (let ((labels-blocks (make-hash-table))
	...
	(let* ((label-block (make-new-ssa-block lambda-ssa))
	       (label (clcomp::label-node-label node))
	       (label-gen (generate-label-for-string (symbol-name label))))
	  (ssa-add-block-label lambda-ssa label-block label label-gen)
		  ...
		  (let* ((align-slot-needed (not (evenp stack-vals)))
		    ...
		    (emit-ir (make-ssa-embedded-instr
			      :instruction (list :mov *fun-number-of-arguments-reg* (@ *base-pointer-reg*
										       ...
										       clcomp::*word-size*)))
			     block)
		    ;; Next sequence will use temporary reg if it's move between two memory locations
		    ...
		    ;; this means that we can't leave this allocation to allocation process
...
(defun maybe-insert-or-fix-jump-instr (from-block to-block &optional (slabel "IF-FBLOCK"))
  ...
  (let ((label (ssa-block-label to-block)))
    (unless label
      (debug-blocks-print "MAYBE-INSERT-OR-FIX-JUMP-INSTR: Creating new label for block " (ssa-block-index to-block))
    ...
    (let ((jump-instruction (is-last-instr-jump from-block)))
	  ...
	  (progn
	    (debug-blocks-print
	     "MAYBE-INSERT-OR-FIX-JUMP-INSTR: Fixing JUMP instruction in block " (ssa-block-index from-block))
	    (if (ssa-go-p jump-instruction)
		(setf (ssa-go-label jump-instruction) label)
		(setf (ssa-if-false-block-label jump-instruction) label)))
	  (progn
	    (debug-blocks-print
	     "MAYBE-INSERT-OR-FIX-JUMP-INSTR: Fixing block jump, emiting SSA-GO for block "
...
(defun maybe-remove-redundant-blocks (sblocks lambda-ssa)
  ...
  (dolist (sblock-cons sblocks)
    (let* ((sblock (car sblock-cons))
	   ...
	   ;; here we now that it's only one successor
      ...
      (unless
	  ;; We search through predecessors, if we found that one of predecessors have our *redundant* block
	  ;; in succ but uncond-jump is filled then we can't remove block
	  ;; This can't happen though
	  ;; So, when predecessor have our block as :succ then we change it to :uncond-jump
	  ;; if :cond-jump or :uncond-jump we just fix index
	  (dolist (pcons predecessors-types)
	    (when (and (eq (car pcons) 'succ)
		       (ssa-block-uncond-jump (ssa-find-block-by-index lambda-ssa (cdr pcons))))
	      (debug-blocks-print
	       (format t "Skipping to remove block ~A, SUCC + UNCOND-JUMP case from ~A" (ssa-block-index sblock)
		       (car pcons)))
	      (error "This should not happen")
	      (return t))
	    (when (= (cdr pcons) (ssa-block-index sblock))
	      ;; WHAT ?
	      (debug-blocks-print (format t "Skipping to remove block ~A, jump to self" (ssa-block-index sblock)))
	      (return t)))
	;; we are clear to remove our block
	(debug-blocks-print "Removing REDUNDANT BLOCK " (ssa-block-index sblock))
	;; replace predecessors jumps with empty block successor
	(dolist (pcons predecessors-types)
	  (let* ((pblock (cdr pcons))
	...
	(let ((del-block-index (ssa-block-index sblock))
	      (succ-block (ssa-find-block-by-index lambda-ssa succ-index))
	      (new-predecessors (mapcar #'cdr predecessors-types)))
	  (setf (ssa-block-predecessors succ-block)
		(append new-predecessors
			(delete del-block-index (ssa-block-predecessors succ-block)))))))))

(defun is-block-maybe-redundant (sblock)
  "Decide if block can be deleted.
   Block maybe can be deleted if there is LABEL instruction before SSA-VALUE or SSA-GO inst.
  ...
  (when (= 1 (length (ssa-block-successors-indexes sblock)))
	...
	(typecase instr
	  ...
	  (ssa-value (if (eq last 'ssa-go)
			 ...
			 (setf last 'ssa-value)))
	  (t (return-from is-block-maybe-redundant nil))))
      (or last 'empty))))
...
(defun remove-redundant-blocks (lambda-ssa)
 ...
 There is also case when block only contain one GO instruction, we can also delete this block and fix predecessors/successors connections"
  (let (maybe-redundant-blocks)
    (dolist (sblock (lambda-ssa-blocks lambda-ssa))
      (let ((redundant (is-block-maybe-redundant sblock)))
	(when redundant
...
(defun remove-not-accessible-blocks (lambda-ssa)
  (let* ((blocks (lambda-ssa-blocks lambda-ssa))
	...
	(if (gethash (ssa-block-index block) visited)
	    ...
	    (progn
	      (debug-blocks-print "REMOVE-NOT-ACCESSIBLE-BLOCKS: Removing block: " (ssa-block-index block))
	      (dolist (sblock (ssa-block-successors block lambda-ssa))
		(when sblock
	...
	(dolist (index (ssa-block-successors-indexes block))
	  (let ((sblock (ssa-find-block-by-index lambda-ssa index)))
	    (when sblock
	      (setf (ssa-block-predecessors sblock)
		    (remove (ssa-block-index block) (ssa-block-predecessors sblock))))))
	(debug-blocks-print "REMOVE-NOT-ACCESSIBLE-BLOCKS: What, Removing again block: " (ssa-block-index block))
	(destroy-ssa-block block lambda-ssa)))))

(defun ssa-block-change-succ-to-uncond-jump (lambda-ssa block)
  (let* ((succ-index (ssa-block-succ block))
	 (succ-block (ssa-find-block-by-index lambda-ssa succ-index))
	 (last-instr (ssa-block-last-instruction block)))
    (assert (= (1+ (ssa-block-order block))
	       (ssa-block-order succ-block)))
    (unless (ssa-block-label succ-block)
      (let ((succ-block-label (generate-label-for-string "SUCC-BLOCK-LABEL-")))
	(label-ssa-block succ-block succ-block-label)))
    (if (typep last-instr 'ssa-if)
	(progn
  ...
  (flet ((change-uncond-jump-to-succ (ssa-block)
	   ...
	   (let* ((last-instruction (ssa-block-ir-last-instr ssa-block)))
	     (typecase last-instruction
	       ...
	       (t (error "Unknown last instruction"))))))
    (let ((blocks (lambda-ssa-blocks lambda-ssa)))
      (do* ((bs blocks (cdr bs))
	...
	(let ((b1-succ (ssa-block-succ b1))
	  ...
	  (cond ((and (null b1-succ)
		 ...
		 (debug-blocks-print "Fixing UNCOND-JUMP to SUCC for " (ssa-block-index b1) (ssa-block-index b2)))
		((and b1-succ
		      b2
...
(defun lambda-ssa-insert-block-in-between-cond-jump (lambda-ssa from-block to-block)
  ...
  (assert (= (ssa-block-cond-jump from-block)
  ...
  (let ((new-block (make-new-ssa-block lambda-ssa))
    ...
    (setf (ssa-block-predecessors to-block)
	  (substitute (ssa-block-index new-block)
		      (ssa-block-index from-block)
		      (ssa-block-predecessors to-block)))
...
(defun cbo-visit (block previous-block lambda-ssa visited active)
  (declare (optimize (debug 3) (speed 0)))
  (debug-print "CBO-VISIT " (ssa-block-index block))
  (let* ((index (ssa-block-index block))
    ...
    (if (> b-active 0)
	(progn
	  (debug-print "SSA-BLOCK-IS-HEADER " (ssa-block-index block) "END BLOCK " (ssa-block-index previous-block))
...
(defun interval-live-at-index-p (interval index)
  (dolist (range (interval-ranges interval))
    (when (and (>= index (range-start range))
	       (< index (range-end range)))
      (return-from interval-live-at-index-p t))))
...
(defun add-use-positions (use-positions virtual use-position)
  (setf (gethash virtual use-positions)
	(cons use-position
	      (gethash virtual use-positions))))
...
(defun ranges-merge-loop (range current-range rest-ranges &optional first-merge result-ranges)
  #.*fun-optimize-level*
  (if current-range
      (let ((merge (intervals-merge (range-start current-range) (range-end current-range)
				    (range-start range) (range-end range))))
	(if merge
	    (let ((new-range (make-range :start (first merge) :end (second merge))))
	      (ranges-merge-loop new-range (car rest-ranges) (cdr rest-ranges) t result-ranges))
	    (if first-merge
		(append (reverse result-ranges) (cons range (cons current-range rest-ranges)))
		(ranges-merge-loop range (car rest-ranges) (cdr rest-ranges) nil (cons current-range result-ranges)))))
      (cons range (reverse result-ranges))))

(defun add-range-loop (interval start end)
  #.*fun-optimize-level*
  (let ((ranges (interval-ranges interval)))
    (sort  (ranges-merge-loop (make-range :start start :end end) (car ranges) (cdr ranges))
	   (lambda (r1 r2)
...
(defun add-range (intervals name start end)
  ...
  (let ((interval (get-interval intervals name)))
    (if interval
	(setf (interval-ranges interval) (add-range-loop interval start end))
	(let ((interval (make-interval :name name :ranges (list (make-range :start start :end end)))))
	  (add-interval intervals interval)))))

(defun shorten-current-range (intervals place start)
  #.*fun-optimize-level*
  (let* ((interval (get-interval intervals place)))
    (if interval
...
(defun range-split (range position)
  (if (= position (range-start range))
      (values nil range)
      (if (and (> position (range-start range))
	       (< position (range-end range)))
	  (let ((old-range (make-range :start (range-start range) :end position))
  ...
  (let ((range-position (position position (interval-ranges interval)
				  :test (lambda (v e)
					  (and (>= v (range-start e))
					       (< v (range-end e)))))))
    (if range-position
	(let* ((interval-ranges (interval-ranges interval))
	    ...
	    (when second-range
	      (setf (interval-ranges interval)
		    (append
			 ...
			 (list first-range)
			 (list)))))
	    (values (when (interval-ranges interval)
		      interval)
		    (when second-range
...
(defun build-intervals (lambda-ssa)
  ...
  (let ((intervals (make-intervals))
	...
	(let ((start (ssa-form-index (ssa-block-first-instruction b)))
	      (end (+ 2 (ssa-form-index (ssa-block-last-instruction b))))
	      (live-out (ssa-block-live-out b)))

	  (dolist (place live-out)
	    (add-range intervals place start end))

	  ;; FIXME, SSA-PLACE currently only works on VIRTUAL-PLACE and PHI-PLACE
	  ;; extend this logic to handle all the places (physical registers too)
	  ;; Look at "fixed intervals" from the "Linear scan" paper
	  ...
	  (dolist (instr (reverse (ssa-block-ssa b)))
		...
		(dolist (write (if (listp writes)
		  ...
		  (let ((write (get-maybe-reduced-place write)))
		    (unless (shorten-current-range intervals write instr-index)
		      ;; NOTE, this is for just one write to place that is never read
		      ;; we sure need to allocate register for this
		      (add-range intervals write instr-index (+ 1 instr-index)))
		...
		(dolist (orig-read (if (listp reads) reads (list reads)))
		  (let ((read (get-maybe-reduced-place orig-read)))
		    (add-range intervals read start instr-index)
...
(defun alloc-remove-active (alloc interval)
  (alloc-remove-active-by-number alloc (interval-number interval)))

(defun alloc-remove-active-by-number (alloc int-number)
...
(defun alloc-remove-inactive (alloc interval)
  (alloc-remove-inactive-by-number alloc (interval-number interval)))

(defun alloc-remove-inactive-by-number (alloc int-number)
...
(defun find-interval-that-use-reg (intervals register)
  (let (res)
    (dolist (i intervals)
      (when (eq register (interval-register i))
	(push i res)))
    res))
...
(defun try-allocate-free-reg (current-interval current-position alloc)
  ...
  (let ((fup (make-positions)))
    ...
    (dolist (interval (alloc-inactive alloc))
      (let ((intersection-index (intervals-first-intersection current-interval interval)))
	(when intersection-index
	  (add-position fup intersection-index (interval-register interval)))))
    ...
    (let* ((reg-pair (get-register-with-max-position fup))
      ...
      (cond ((zerop reg-pos)
	    ...
	    ((<= (interval-end current-interval) reg-pos)
	     ...
	     t)
	    ;; if interval starts exactly at this position then we can't do anything here
	    ;; if we add interval to unhandled we can enter infinite loop
	    ((<= reg-pos (interval-start current-interval))
	     nil)
	    (t
...
;; FIXME, in DOLIST, should we check if interval has REG as alocation (it's not possible, we are adding stack intervals to handled)
(defun allocate-blocked-reg (current-interval alloc)
  ...
  (let ((fup (make-positions))
    ...
    (dolist (interval (alloc-active alloc))
      (add-position fup (interval-get-next-use interval current-index) (interval-register interval)))
    ...
    (let* ((reg-pair (get-register-with-max-position fup))
      ...
      (cond ((>= current-first-usage reg-pos)
	     ;; paper here say that we split on index where REG is required
	     ;; in our current implementation this is not required
	     ;; TODO, fix later when we have stable implementation
	     (spill-interval alloc current-interval))
	    (t
	     ;; spill interval that blocks the reg
	     (let ((active-reg-intervals (find-interval-that-use-reg (alloc-active alloc) reg)))
	       (assert (= 1 (length active-reg-intervals)))
	       (let* ((active-interval (first active-reg-intervals))
		      (active-int-num (interval-number active-interval)))
		 (multiple-value-bind (int sint) (split-interval active-interval current-index)
		   ;; since this is still active interval we should always have SINT
		   ;; if it starts at the same POSITION as CURRENT-INTERVAL then it will not have INT
		   (assert sint)
		   (if int
		       (progn
		   ...
		   (spill-interval alloc sint)))
	       (setf (interval-register current-interval) reg)
	       (alloc-add-active alloc current-interval)
	       (let ((inactive-reg-intervals (find-interval-that-use-reg (alloc-inactive alloc) reg)))
		 (dolist (inactive-interval inactive-reg-intervals)
		   (let ((next-range (interval-first-range-after-index inactive-interval current-index))
			 (next-intersection (intervals-first-intersection current-interval  inactive-interval)))
		     (when (and next-range next-intersection)
...
(defun linear-scan (intervals)
  #.*fun-optimize-level*
  (let ((alloc (make-alloc :unhandled intervals :stack-index -1)))
    (tagbody
     START
	 ...
	 (when current-interval
	   ...
	   (progn
		 ...
		 (cond ((<= (interval-end ia-interval) position)
	       ...
	       (let ((allocated (try-allocate-free-reg current-interval position alloc)))
		 (unless allocated
...
(defun resolve-data-moves-order (moves scratch-storage)
  ...
  (if (< (length moves) 2)
      ...
      (let ((stack-moves nil)
	    (non-stack-moves nil))
	(dolist (move moves)
	  (if (eq :stack (operand-place-type (ssa-load-to move)))
...
(defun resolve-lambda-moves-order (lambda-ssa scratch-storage)
  #.*fun-optimize-level*
  (dolist (b (lambda-ssa-blocks lambda-ssa))
    (setf (ssa-block-cond-jump-moves b)
	  (resolve-data-moves-order (ssa-block-cond-jump-moves b) scratch-storage))
    (setf (ssa-block-uncond-jump-moves b)
	  (resolve-data-moves-order (ssa-block-uncond-jump-moves b) scratch-storage))
    (setf (ssa-block-succ-moves b)
	  (resolve-data-moves-order (ssa-block-succ-moves b) scratch-storage))
    ;; spill moves are all write to stack slot + are indexed, no need for any order
    (setf (ssa-block-spill-moves b)
	  (resolve-indexed-data-moves-order (ssa-block-spill-moves b) scratch-storage))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;; Merging PHI moves and spill moves
...
(defun block-merge-moves (lambda-ssa sblock)
  ...
  (if (= 1 (length (ssa-block-successors-indexes  sblock)))
	...
	(merge-simple-edge-moves sblock (first moves)))
      ...
      ;; COND + UNCOND (IF with both true and false block labels)
      ;; UNCOND + SUCC (currently we should now have state like this but semantically it's possible ??)
      (let ((succ-moves (ssa-block-succ-moves sblock))
	    (cond-moves (ssa-block-cond-jump-moves sblock))
	    (uncond-moves (ssa-block-uncond-jump-moves sblock)))
	(assert (<= (length (remove nil (list succ-moves cond-moves uncond-moves)))
		    2))
...
(defun get-fixed-place-storage (place)
  ...
  (etypecase place
    ...
    (operand-place (operand-place-operand place))))
...
(defun get-alloc-storage (alloc name index)
  ...
  (let ((intervals (gethash name (alloc-per-name-handled alloc))))
	...
	(when (and (<= (range-start range) index)
	  ...
	  (let ((reg (interval-register interval))
		(stack (interval-stack interval)))
	    (return-from get-alloc-storage (if reg
					       (make-reg-op reg)
					       (make-stack-op (calculate-local-var-stack stack))))))))
